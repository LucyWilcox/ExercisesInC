...
Yep, the threads often read and print the same value even thought they increment the correct (not the same) one. Also with 200 threads I got an error where the final value was 199 so two threads incremented the same value.
...
so the real overhead is 0.076s   